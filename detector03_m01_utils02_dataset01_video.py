...
class Detector(torch.nn.Module):
    ...
    def forward(self, input, thresh, anchors):
        ...
        time_nms2=time.time()
        print('time_nms:',time_nms2-time_nms)
    ...
    def resize_img(self,image):
        ...
        time_resize2=time.time()
        print('time_resize:',time_resize2-time_resize)
...
if __name__ == '__main__':
    ...
    # # (1280, 720)是视频的分辨率(最好与图片大小一样)
    # videoWriter = cv2.VideoWriter("video/01.avi", fourcc, fps, (1280, 720))

    # 循环，每次回读取视频一帧的图片
    while rval:
        c = c + 1
        # c除以2取余为0时直接跳过，即两帧放入MTCNN框一次
        if c % 2 == 0:
            continue
        else:
            ...
            for box in y:
                ...
                # draw.text((x1, y1), text=str(cls_dict[int(box[6])]) + ' : ' + str(round(float(box[1]), 4)), font=font,
                #           fill=(0, 255, 255))
                ...
                draw1.rectangle((x1_, y1_, x2_, y2_), outline=color[int(box[6])], width=2)
                draw1.text((x1_, y1_), text=str(cls_dict[int(box[6])]) + ' : ' + str(round(float(box[1]), 4)),
                           font=font, fill=(0, 255, 255))

            time_pic2=time.time()
            print('time_pic:',time_pic2-time_pic)
            ...
            cv2.imshow(winname='video', mat=cv_img)
            # 保存图片
            # cv2.imwrite(r'video/'+str(c)+'.jpg',frame)
            # 写成视频
