...
module02是在网络层中将置信度、中心点、宽高、分类全切出来加上激活
效果：训练的更慢
'''

from YOLOV3.Two_target import dataset01
from YOLOV3.Two_target.module02 import *
import os
import torch
...
def loss_fn(output, target, alpha):
    ...
    在网络层中加的，所以在测试时，不需要再加sigmiod
    '''
    out_iou_obj=output[mask_obj][:,0].float()
    target_iou_obj=target[mask_obj][:,0].float()
    iou_loss_obj=bceloss(out_iou_obj,target_iou_obj)
    # print(iou_loss_obj)

    out_iou_noobj=output[mask_noobj][:,0].float()
    ...
    在网络层中加的，所以在测试时，不需要再加sigmiod
    '''
    out_center_obj=output[mask_obj][:,1:3].float()
    ...
    在网络层中加了softmax了，所以在测试时，不需要再加softmax
    注：这里用到nllloss，所以要加softmax，如果用sofmax交叉熵，其实在网络输出层也不需要加softmax了
    '''
    out_cls_obj=output[mask_obj][:,5:]
    # 由于制作样本时对应的分类标签使用的是one-hot，而传入交叉熵损失和Nullloss的是直接的标签形式，所以要用torch.argmax(target[mask_obj][:,5:],dim=1)来求出具体类别才行
    target_cls_obj=torch.argmax(target[mask_obj][:,5:],dim=1).long()
...
if __name__ == '__main__':
    ...
    model_path='models/yolov3_two04_02_01.pth'
